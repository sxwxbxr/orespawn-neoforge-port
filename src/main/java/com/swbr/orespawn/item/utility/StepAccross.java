...
/**
 * Port of {@code danger.orespawn.StepAccross} (StepAccross.java:13-108): {@code step_accross}
 * "Insta-Bridge" (OreSpawnMain.java:1592), stack 16, creative tab {@code tabTools} (:15-18). The
 * level twin of {@link StepUp}: cobblestone at {@code y - 1} for up to 32 blocks, the torch on top
 * at {@code y}, stopped by the first non-air block.
 */
public class StepAccross extends Item {

    /** {@code StepAccross(id)} (:15-18): {@code maxStackSize = 16}. */
    public StepAccross(final Item.Properties props) {
    ...
    public InteractionResult useOn(final UseOnContext context) {
        ...
        for (int k = 1; k < length; ++k) { // :85
            ...
            BlockState bid = world.getBlockState(new BlockPos(bx, y - 1, bz)); // :86
            if (!OneUse.isAir(bid)) { // :87-89
                break;
            }
            FastBlocks.setBlockFast(world, bx, y - 1, bz, cobblestone, Block.UPDATE_CLIENTS); // :90
            if ((k - 1) % 8 == 0) { // :91
                bid = world.getBlockState(new BlockPos(bx, y, bz)); // :92
                if (OneUse.isAir(bid)) { // :93-95
                    FastBlocks.setBlockFast(world, bx, y, bz, ModBlocks.EXTREME_TORCH.get().defaultBlockState(), Block.UPDATE_CLIENTS);
                }
            }
        }
