...
/**
 * Keeps the 1.7.10 order of a right click on {@link Dragon} and {@link Spyro} - the restoration W06's
 * {@code CannonFodderInteractEvents} does for its family.
 *
 ...
 *
 * <p>What would change otherwise:
 * <ul>
 *   <li>The owner's name tag reached {@code interact} first and named the dragon <em>without</em> persistence. With the
 *       vanilla tag first, a named Baby Dragon would become persistent and never grow into a Dragon
 *       (Spyro.java:449 requires {@code !isNoDespawnRequired()}).</li>
 *   <li>A stranger's lead on a tame dragon went to {@code interact}, which has no branch for it, and did not attach.</li>
 * </ul>
 * {@code PlayerInteractEvent.EntityInteract} fires in {@code Player.interactOn} ahead of {@code Mob.interact}. Every
 * other click takes the vanilla path, whose leash-then-{@code mobInteract} order matches.
 */
@EventBusSubscriber(modid = OreSpawn.MOD_ID)
public final class DragonInteractEvents {

    private DragonInteractEvents() {}
    ...
    public static void onEntityInteract(final PlayerInteractEvent.EntityInteract event) {
        ...
        if (!(target instanceof Dragon) && !(target instanceof Spyro)) {
            return;
        }
        ...
        final InteractionResult result = animal instanceof Dragon dragon
                ? dragon.mobInteract(player, event.getHand())
                : ((Spyro) animal).mobInteract(player, event.getHand());
