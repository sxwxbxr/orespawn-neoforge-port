...
/**
 * Port of {@code danger.orespawn.BlockMothPlant} (BlockMothPlant.java:13-83), id {@code moth_plant},
 * planted by {@code moth_seed}. {@link BlockButterflyPlant} with two differences: it hatches at
 * <em>night</em> and under {@code MothEnable} (:37), one "Moth" (:38). Icons {@code moth_0..3}, drops
 * {@code 1 + rand(5)} seeds unripe, the vanilla seed rolls ripe.
 */
public class BlockMothPlant extends CropBlock {

    public static final MapCodec<BlockMothPlant> CODEC = simpleCodec(BlockMothPlant::new);
    ...
    public BlockMothPlant(final BlockBehaviour.Properties properties) {
        super(properties);
    }

    @Override
    public MapCodec<BlockMothPlant> codec() {
        return CODEC;
    }
    ...
    protected void randomTick(final BlockState state, final ServerLevel par1World, final BlockPos pos, final RandomSource par5Random) {
        ...
        if (bid.isAir() && !InsectSupport.isDaytime(par1World) && MobSwitches.enabled(OreSpawnConfig.MOBS.MothEnable)) {
            spawnCreature(par1World, ModEntities.MOTH.get(), pos.getX() + 0.5, pos.getY() + 1.01, pos.getZ() + 0.5);
        }
    }
